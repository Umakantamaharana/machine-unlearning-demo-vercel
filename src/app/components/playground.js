...
const Playground = ({ addCommand, updateInfo, updateReference }) => {
  ...
  const models = ["AllCNN", "ResNet18", "MobileNet"];
  ...
  const stateChange = () => {
    ...
    setTimeout(() => {
      if (buttonId === 0) {
        updateInfo(
          'Unzip and Load the Dataset. Click on the "Load" button to start the process. This will 1. Unzip the dataset file. 2. Apply your user-defined preprocessing function to each image. 3. Load the preprocessed images for further analysis.Unzip and preprocess dataset using a user-defined function.'
        );
      } else if (buttonId === 1) {
        updateInfo(
        ...
        updateInfo(
          "Training has been finished. Now you can select data to be forgotten by the model."
        );
      } else if (buttonId === 4) {
        updateInfo(
          "Now you can test the model to check loss of both forget and retain dataset. Click on evaluate button."
        );
      } else if (buttonId === 5) {
        updateInfo(
        ...
        updateInfo(
          "Model has unlearned the forget set of data. Now train a gold standard model."
        );
      }
      // else if (buttonId === 8) {
      ...
      <div className="playground mt-2 h-full w-full border border-green-700 rounded-md flex justify-center items-center relative">
        ...
        <div className="flex flex-col items-center justify-center absolute bottom-0 right-10">
          <span style={{ fontSize: "15px", margin: 0, padding: 0 }}>
            ...
            &{" "}
            <a href="https://umakantamaharana.github.io" target="_blank">
              Umakanta Maharana
            </a>
          </span>
        </div>
